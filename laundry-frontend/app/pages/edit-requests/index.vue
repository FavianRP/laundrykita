...
async function load() {
    ...
    try {
        ...
        const res = await api.get<Paginated<EditRequest>>(
            "/edit-requests",
            params,
        );
...
<template>
    <div>
        ...
        <h1 class="font-display text-3xl text-[var(--color-ink)] mb-6">
            Pengajuan Perbaikan &amp; Pembatalan
        </h1>
        ...
        <div class="flex gap-2 mb-5">
            <button
                ...
                :class="
                    statusFilter === s
                        ? 'bg-[var(--color-soap)] text-white border-[var(--color-soap)]'
                        : 'border-[var(--color-line)] text-[var(--color-ink-soft)] hover:border-[var(--color-soap)]'
                "
        ...
        <div v-else class="space-y-3">
            ...
            >
                <div class="flex items-center justify-between gap-3 flex-wrap">
                    <div>
                        <p class="font-medium text-[var(--color-ink)]">
                            {{
                                r.is_cancellation_request
                                    ? "Permintaan Pembatalan"
                                    : "Permintaan Perbaikan Data"
                            }}
                            <span
                                class="text-[var(--color-ink-soft)] font-normal"
                                >· Order #{{ r.order_id }}</span
                            >
                        </p>
                ...
                <p
                    class="text-[11px] font-mono text-[var(--color-ink-soft)] mt-2"
                >
